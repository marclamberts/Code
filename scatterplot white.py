...
defensive = pd.read_excel('BEL Strikers.xlsx')
defensive.head()
...
# defenders
attackersfilter = (attackersfinal['Team'] != 'Arsenal') | \
        (attackersfinal['Player']!= 'T. Arokodare')
attackersd = attackersfinal[attackersfilter]

teamfilter = (attackersfinal['Team'] == 'Mvv')
teamd = attackersfinal[teamfilter]

# Sergio Reguilon
randallfilter = (attackersfinal['Player'] == 'C. De Ketelaere')
...
ax.spines['bottom'].set_color('xkcd:black')
ax.spines['top'].set_color('xkcd:black')
ax.spines['left'].set_color('xkcd:black')
ax.spines['right'].set_color('xkcd:black')
...
plt.text(1.075,0.5,"\n", \
        horizontalalignment='right', verticalalignment='top', color='xkcd:black', size='18', transform=ax.transAxes)
...
words = "De Ketelaere vs Pro League strikers 2021/2022".split()
colors = ['xkcd:bright green', 'xkcd:bright green', 'xkcd:black', 'xkcd:black', 'xkcd:black',
          'xkcd:black', 'xkcd:black']
rainbow_text(.25, .508, words, colors, size=18)


# Adding graph and axes titles
ax.set_xlabel('\nxG per 90\n', color='xkcd:black', size = 15)
ax.set_ylabel('\nxA per 90', color='xkcd:black', size = 15)
ax.set_title("\nExpected goals per 90 vs Expected assists per 90\n", color='xkcd:black', size=25)
...
plt.axhline(y=y_mean, xmin =-100, xmax=100, color='xkcd:black', linestyle='--', linewidth=2)
plt.axvline(x=x_mean, ymin=-100,ymax=100, color='xkcd:black', linestyle = '--', linewidth=2, zorder=0.2)
ax.fill_between([x_mean, 1000], y_mean,1000, alpha=0.3, color='xkcd:light green', zorder=10)
...
for i in n2:
    if i != 'S. Blackstenius':
        plt.annotate(i, (x2[n2.index(i)], y2[n2.index(i)]+0.02), color='xkcd:black', \
        horizontalalignment='center', fontsize=15, zorder=100)   
#for i in n1:
 #   if i == 'L. Smans':
  #      plt.annotate(i, (x1[n1.index(i)], y1[n1.index(i)]+0.05), color='xkcd:off white', \
   #3     horizontalalignment='center', fontsize=12, zorder=100)  
for i in n3:
        plt.annotate(i, (x3[n3.index(i)], y3[n3.index(i)]+0.01), color='xkcd:green', \
        horizontalalignment='left', fontsize=15, zorder=100) 
 
 # Watermarking visualization
plt.text(0.03, -0.07, 'Twitter @lambertsmarc\nData from Wyscout', \
         horizontalalignment='left', verticalalignment='top', color='xkcd:black', transform=ax.transAxes,zorder=1000)
plt.show()

plt.savefig('scat.png',dpi=1500,bbox_inches = 'tight',facecolor='white')
