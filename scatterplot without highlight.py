...
defensive = pd.read_excel('ERE MID.xlsx')
defensive.head()
...
attackersfinal['Assists per 90'] = defensive['Assists per 90']
...
x_mean = np.mean(attackersfinal['xA per 90'])
y_mean = np.mean(attackersfinal['Assists per 90'])
...
# Sergio Reguilon
randallfilter = (attackersfinal['Player'] == 'O. Kökçü')
...
x1 = list(attackersd['xA per 90'])
x3 = list(randalld['xA per 90'])
x2 = list(teamd['xA per 90'])
y1 = list(attackersd['Assists per 90'])
y3 = list(randalld['Assists per 90'])
y2 = list(teamd['Assists per 90'])
...
words = "Orkun Kökçü vs Eredivisie 2021/2022".split()
colors = ['xkcd:bright green', 'xkcd:bright green', 'xkcd:off white', 'xkcd:off white', 'xkcd:off white',
          'xkcd:off white', 'xkcd:off white']
rainbow_text(0.11, 0.66, words, colors, size=18)


# Adding graph and axes titles
ax.set_xlabel('\nxA per 90\n', color='xkcd:off white', size = 15)
ax.set_ylabel('\nAssists per 90', color='xkcd:off white', size = 15)
ax.set_title("\nxA per 90 vs Assists per 90\n", color='xkcd:off white', size=25)
...
for i in n2:
    if i != 'O. Kökçü':
        plt.annotate(i, (x2[n2.index(i)], y2[n2.index(i)]+0.03), color='xkcd:off white', \
