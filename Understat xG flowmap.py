...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 16 16:35:46 2022
...
link = "https://understat.com/match/16741"
...
ax.step(x = df_home['minute'] ,y = df_home['xGcum'] , where = 'post', color = '#5C2180' ,linewidth = 4.0)
ax.step(x = df_away['minute'] ,y = df_away['xGcum'] , where = 'post', color = '#FDBE11' ,linewidth = 4.0)

#sns.scatterplot(x=x,y=y,s=430,marker='o',color='yellow')
ax.scatter(x=x,y=y, color='#5C2180', edgecolor='black',s=955, label="Fiorentina",linewidths=1.5,)
ax.scatter(x=x1,y=y1, color='#FDBE11', edgecolor='black',s=955, label="Roma",linewidths=1.5)

#FILL AREA BETWEEN LINE AND X 
plt.fill_between(x,y, alpha=0.08, color='#5C2180')
plt.fill_between(x1,y1, alpha=0.08, color='#FDBE11')
...
# title
#fig_text(0.08,1.03, s="xG Flowchart Premier League\n", fontsize = 35, fontweight = "light")
fig_text(0.08,1, s=" <{} {} xG> vs <{} {} xG>".format(team_home,xGcum_home,team_away,xGcum_away), fontsize = 20, fontweight="light", c="black")
#fig_text(0.8,1.03, s="@lambertsmarc\n", fontsize = 25, fontweight = "light")
...
plt.vlines( ymin=0, ymax=2,x=45, color='black', alpha=0.1,linestyle="solid")
# ticks
...
# Home team
for i in range(len(x)):
    plt.annotate(y_plot[i], (x[i], y[i]),c='white',size=12,ha='center',va='center',fontweight='bold')
# Away team
for i in range(len(x1)):
...
plt.savefig('sas1.png',dpi=500,bbox_inches = 'tight', facecolor='white')
